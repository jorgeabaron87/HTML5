...
function init(){
	// Set uo the Box2d world that will do most of the physics calculation
	var gravity = new b2Vec2(0,9.8); // declare gravity m/s^2 downward
	var allowSleep = true; // Allow objects thar are at rest to fall asleep an be excluded from calculations
...
// As per the Box2d manual, the suggested iteration count for Box2 is 8 for velocity and 3 for position.
...
function animate(){
	...
	if(specialBody && specialBody.GetUserData().life <=0){
		...
		console.log("The SpecialBdoy was destroyed");
	}
...
function createFloor(){
	// A body definition holds all the data neede to construct a rigid body.
	var bodyDef = new b2BodyDef;
	bodyDef.type = b2Body.b2_staticBody;
	bodyDef.position.x = 640/2/scale;
	bodyDef.position.y = 450/scale;

	// A fixure is used to attach a shape to a body for collision detection
...
function createSpecialBody(){
	...
	//Create fixture to attach a circular shape to the body
	var fixtureDef = new b2FixtureDef;
	fixtureDef.density = 1.0;
	fixtureDef.friction = 0.5;
